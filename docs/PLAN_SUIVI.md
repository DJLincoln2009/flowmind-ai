...
| 2026-09-13 | Fix auth manquante en prod (déploiement) | App déployée : aucun UI de connexion → 401 en boucle sur `/api/stats` et `/api/workflows/folders`, lien sidebar `/settings` mort (404 RSC). Ajout `app/auth/login` (login + inscription, i18n FR/EN), garde d'auth synchrone dans le layout `(dashboard)` (pas d'attaches sans token), bouton « Se déconnecter », retrait du lien mort, redirect auto vers `/auth/login` à l'échec du refresh token. Groupe `(auth)` abandonné (route non résolue par le toolchain) au profit de `app/auth/`. E2E smoke vert : garde, inscription → dashboard sans 401, logout, login |
